...
def positive_accuracy(y_true,y_pred):
    one_indices=K.cast(K.where(K.equal(y_true,1.0)),'int32')
    y_true_subset=K.gather_nd(y_true,one_indices)
    y_pred_subset=K.gather_nd(y_pred,one_indices)
...
def negative_accuracy(y_true,y_pred):
    one_indices=K.cast(K.where(K.equal(y_true,0.0)),'int32')
    y_true_subset=K.gather_nd(y_true,one_indices)
    y_pred_subset=K.gather_nd(y_pred,one_indices)
